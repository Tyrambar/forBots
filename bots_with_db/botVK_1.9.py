...
def step_work_with_e(user, curr):
    ...
    def step_e(numb = None):
        ...
        if curr in events:
            ...
            text = ru_texts['format_event_repr']\
                (event_attrs[:-2] + (get_name_by_id(event_attrs[-2]),), ru_texts)
            m_send(user, text, menu)
        # Case, when response is event serial number
            ...
            if users[user].see_my_e_lst:
                ...
                if get_sign_db(users[user].db_id, users[user].previous):
                    ...
                    menu = make_menu(user, [ru_texts['agree']])
                event_attrs = execute_query(conn, GET_EVENT_ALL, (users[user].previous,)).fetchone()
            else:
                ...
                if get_sign_db(users[user].db_id, events[numb-1]):
                    ...
                    menu = make_menu(user, [ru_texts['agree']])
                event_attrs = execute_query(conn, GET_EVENT_ALL, (events[numb-1],)).fetchone()
            text = ru_texts['format_event_repr']\
                (event_attrs[:-2] + (get_name_by_id(event_attrs[-2]),), ru_texts)
            m_send(user, text, menu)
        else:
    ...
    def step_confirm():
        ...
        if not get_sign_db(users[user].db_id, users[user].previous):
            eve_id = execute_query(conn, GET_ID_EVENT_BY_NAME, (users[user].previous,)).fetchone()[0]
            execute_query(conn, ADD_SIGNS, (users[user].db_id, eve_id))
            host_curr_id = execute_query(conn, GET_HOST_ID_ALL, (users[user].previous,)).fetchone()[0]
            m_send(host_curr_id,
                   ru_texts['step_conf_txt'](get_name_by_id(user), users[user].previous))
    ...
    def step_canc():
        ...
        host_curr_id = execute_query(conn, GET_HOST_ID_ALL, (users[user].previous,)).fetchone()[0]
        m_send(host_curr_id, ru_texts['step_canc_txt'](get_name_by_id(user),
                                                                    users[user].previous))
    ...
    if users[user].change == -1:
        ...
        return step_confirm()
    elif 'отмен' in curr.lower() and get_sign_db(users[user].db_id, users[user].previous):
        return step_canc()
...
def create_e_f(user, curr):
    ...
    if users[user].deep == 10:
        ...
        add_event_q = create_add_event_q(conn, args_4_create, users[user].db_id)
...
def edit_e_f(user, curr):
    ...
    if users[user].change == 1:
        ...
        host_curr_id = execute_query(conn, GET_HOST_ID_ALL, (curr,)).fetchone()[0]
...
def see_my_host(user):
    ...
    text = ''
    for eve in events:
        host_curr_id = execute_query(conn, GET_HOST_ID_ALL, (eve,)).fetchone()[0]
...
for event in longpoll.listen():
    #try:
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            ...
            if user_id not in {exist_vk_id[0] for exist_vk_id in execute_query(conn, ALL_VK_ID).fetchall()}:
                execute_query(conn, ADD_USER, [user_id, None, random_passw()])
            if user_id not in users:
                found_db_id = execute_query(conn, GET_ID_USER_BY_VK, (user_id,)).fetchone()[0]
            ...
            if re.match(ru_texts['begin'], current.lower()) or current == ru_texts['to_begin']:
                ...
                m_send(user_id, execute_query(conn, GET_PASSW_BY_VK).fetchone()[0])
