...
def step_conf_txt_f(user_name, event_name):
    txt = f"Пользователь {user_name} записался на Ваше " \
          f"мероприятие '{event_name}'"
    return txt


def step_canc_txt_f(user_name, event_name):
    txt = f"Пользователь {user_name} отменил запись на Ваше " \
          f"мероприятие '{event_name}'"
    return txt
...
options = ('Чтобы добавить мероприятие, вам нужно заполнить поэтапно :\n'
           '📌Краткое описание мероприятия, которое будет отображаться на '
           ' кнопке\n📌Адрес места проведения мероприятия. Пример: '
           'Маросейка 13с1\n📌Дата проведения (месяц, дата, час - разделение '
           'через запятую). Пример: 5, 25, 18\n📌Описание места проведения\n')
...
daysweek = {'Monday': 'понедельник', 'Tuesday': 'вторник',
...
hi_from_bot=("Добро пожаловать! Ознакомьтесь с ближайшими мероприятиями.\n"
             "Можете записаться на них, отменить запись и посмотреть "
             "в любое время🙂.\n📌Обратите внимание на значки в поле "
             "сообщений, рядом со смайлом. Нажмите - откроется меню или "
             "список команд.\nЧтобы синхронизировать учетную запись с "
             "Телеграмом/ВК, наберите для получения пароля `get` и затем "
             "`sync`, пробел и пароль"),

choice_e='Выберите мероприятие: напишите порядковый номер или нажмите '
           'на кнопку:\n',
confirm='Вы записались на ',
wrong=('😔 Я не могу вас понять\nВы можете попробовать снова:\n'
       '📌 Откройте рядом со смайликами квадратную иконку для удобной '
...
fail_right_4_edit_e=('Вы не являетесь создателем этого мероприятия, поэтому '
...
begin=(r'(меню)|(старт[уй]*)|(прив[етствую]*)|(добр[огоыйе]+( )?' 
       r'[утроадняденьвечера]*)|([вс]* ?нач[ниалоать]*)|'
       r'(з?д[ао]+р[аоваствуйте]+)|(хай)|(hi)|(hello)|(ку)'),
a_che_tam=r'([а ]*ч[еёо]{1} там\??)|(как [тыдела]\??)',
welcom_re=(r'(с?пас[ие]б[ао]*)|(благодарю?[ствую]*[им]*)|(но?рм[ас]*)|'
           r'(к ?р ?а ?с ?[иа]? ?в ?[ао]?[чик]*)'),
